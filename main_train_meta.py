...
def preprocess_stock_data(
    ...
    validation_size=0.1,
    time_steps=60, 
    scaler_output_file='outputs/meta_scale.pkl'
):
    """
...
data = "inputs/meta_stock_cleaned.csv"
...
class StockPricePredictor_Meta:
    def __init__(self, input_shape, learning_rate=0.04624064196479419, num_layers=2, units=32):
...
data_path_meta = "inputs/meta_stock_cleaned.csv"
...
if __name__ == "__main__":
    ...
    predictor = StockPricePredictor_Meta(input_shape=input_shape)

    # Training
    predictor.train(X_train, y_train, validation_data=(X_val, y_val))

    # Save the model
    model_path = "outputs/stock_price_rnn_meta_model.keras"
    scaler_path = "outputs/meta_scale.pkl"
    predictor.save(model_path)

    # Load model and scaler for inference
    loaded_model, loaded_scaler = StockPricePredictor_Meta.load(model_path, scaler_path)
...
plt.plot(loaded_scaler.inverse_transform(predictions.reshape(-1, 1)), label="Predicted Prices", color="#4267B2", alpha=0.6, linestyle="--", linewidth=2)
plt.title("Stock Price Prediction for Meta",fontsize = 26)
plt.xlabel("Time Steps",fontsize = 20)
plt.ylabel("Stock Price (Original Scale)",fontsize = 20)
plt.legend(fontsize = 20)
plt.grid(True)
plt.savefig('results/meta_result.png', bbox_inches='tight')
plt.show()
