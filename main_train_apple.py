...
def preprocess_stock_data(
    ...
    validation_size=0.1,
    time_steps=60, 
    scaler_output_file='outputs/apple_scale.pkl'
):
    """
...
data = "inputs/apple_stock_cleaned.csv"
...
class StockPricePredictor_Apple:
...
data_path_apple = "inputs/apple_stock_cleaned.csv"
...
if __name__ == "__main__":
    ...
    predictor = StockPricePredictor_Apple(input_shape=input_shape)

    # Training
    predictor.train(X_train, y_train, validation_data=(X_val, y_val))

    # Save the model
    model_path = "outputs/stock_price_lstm_apple_model.keras"
    scaler_path = "outputs/apple_scale.pkl"
    predictor.save(model_path)

    # Load model and scaler for inference
    loaded_model, loaded_scaler = StockPricePredictor_Apple.load(model_path, scaler_path)
...
plt.title("Stock Price Prediction for Apple",fontsize = 26)
plt.xlabel("Time Steps",fontsize = 20)
plt.ylabel("Stock Price (Original Scale)",fontsize = 20)
plt.legend(fontsize = 20)
plt.grid(True)
plt.savefig('results/apple_result.png', bbox_inches='tight')
plt.show()
